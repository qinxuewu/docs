...
`Redis`的所有命令操作都是单线程的，本身提供像`incr`这样的自增命令，所以能保证生成的ID肯定是唯一有序的。

这种方式不依赖关系数据库，而且速度快。但系统要引入Redis这一中间件，增加维护成本，而且编码和配置工作量比较大。即使已经有了`Redis`组件，但生成ID的高频率访问对单线程的Redis性能势必也会造成影响。

还可以利用像`Zookeeper`中的`znode`数据版本来生成序列号，及`MongoDB`的`ObjectId`等
